...
interface Kort {
  ...
  ikon: string;
  tema: string;
  fremtid: string; // deg om 5–10 år (future-self)
  tall: string; // realitetsorientering, plain + absolutt
  kilde: string;
  handling: string; // efficacy: doable + it works
}
...
export function FlowHelsekompetanse({ steg, basePath }: { steg: StegDef[]; basePath: string }) {
  ...
  // Fysisk aktivitet — vises alltid (universelt råd, ikke-stigmatiserende), tilpasset alder
  kort.push(
    eldre
      ? {
          id: "aktivitet",
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        {/* Innramming: ærlig, ikke fordømmende (autonomistøttende) */}
        ...
        {/* Selvrapport — du velger hva som gjelder deg (unngår stigmatiserende antakelser) */}
        ...
        {/* Hvorfor viser vi dette + evidensgrunnlag */}
        <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-4 mb-6">
          <p className="flex items-start gap-2 text-xs text-neutral-600">
            ...
            <span>
              Hvorfor akkurat slik? Forskning på helsekommunikasjon viser at frykt alene virker svakt
              og kan slå tilbake, mens budskap som kobler en ærlig konsekvens til et konkret, gjennomførbart
              tiltak virker. Å knytte det til «deg om 5–10 år» gjør fremtiden nærmere og lettere å handle på.
              Les mer på <a href={`${basePath}/om`} className="text-blueberry-700 hover:underline">Om Helsemelding 1.2</a>.
            </span>
          </p>
        </div>
